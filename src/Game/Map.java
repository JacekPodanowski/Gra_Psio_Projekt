...
public class Map {
    ...
    public void generateMap(Player player) {
        ...
        if (random.nextBoolean()) {
            exitRow = 0;
            exitCol = random.nextInt(map.TabOfRoom.length);
            map.TabOfRoom[0][exitCol].setExit(true);
        } else {
            exitRow = random.nextInt(map.TabOfRoom.length);
            exitCol = map.TabOfRoom.length-1;
            map.TabOfRoom[exitRow][0].setExit(true);
        }
        // temp druk
        ...
        for (int i = 0; i < map.TabOfRoom.length; i++) {
            for (int j = 0; j < map.TabOfRoom.length; j++) {
                map.TabOfRoom[i][j].setNumRoom(numRoom);
                numRoom++;
            }
        }
        ...
        int MapLength = map.TabOfRoom.length; // pomocnicza zmienna
        ...
        int source = map.TabOfRoom[map.TabOfRoom.length-1][0].getNumRoom();
        int dest = map.TabOfRoom[exitRow][exitCol].getNumRoom();
        ...
        for (int i = 0; i < toExit.size(); i++) {
            ToExitRooms[i] = FindRoomByNum(toExit.get(i), map.TabOfRoom);
        }
